...
int main(int argc, char* argv[]) {
	...
	while (1) {
		...
		if (contador >= 2) {
			...
			// A) Inicialize e adicione o socket ao conjunto de sockets dado pelo nome fd_accept
			...

			// B) Configure o timeout dado pelo nome tempoEspera. O valor do timeout é dado pela constante LIMITE_ESPERA
			...

			// C) Preencha corretamente a função select
			switch (select(..., ..., NULL, NULL, ...)) {
			case SOCKET_ERROR:
				printf("<SERV> Erro ao invocar \"select()\" para efeitos de timeout de ligacao (error: %d)!\n");
				continue;
			case 0:
				ocorrenciaTimeout = 1;
				break;
		...
		if (!ocorrenciaTimeout) {
			/*====================== ATENDE PEDIDO ========================*/
			// D) Preencha corretamente a linha seguinte
			if ((... = accept(..., (struct sockaddr*)&cli_addr, &cliaddr_len)) == SOCKET_ERROR) {
	...
	while (1) {
		...
		default:
			for (i = 0; i < grupo->tamGrupo; i++) {
				...
				// E) Preencha corretamente a linha seguinte
				if (...(grupo->sockets[i], ...)) {
...
int difunde(SOCKET origem, SOCKET* grupo, int tam)
{
	...
	// F) Preencha corretamente as linhas seguintes de forma a que receba 1 caracter do socket dado pelo nome "origem"
	//    e o envie para todos os elementos do grupo (excepto para ele próprio)
	if ((result = ...(..., &c, sizeof(char), 0)) == sizeof(char)) {
